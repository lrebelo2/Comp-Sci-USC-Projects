...
// add private data and methods here
  ...
  //Goes through the hash table and computes the longest chain
  unsigned int computeLongestChain() const;

  unsigned int hashSize;      // size of the hash table
                              // (used in hashCode method above)
  unsigned int numEnt;    //number of entries in the hash table
  unsigned int nEBuckets;     //number of non-empty buckets in the hash table
  unsigned int longestChain;  //the longest chain in the hash table. Gets update in every call to insert() and computed at every call to remove()
  ListType * data;
